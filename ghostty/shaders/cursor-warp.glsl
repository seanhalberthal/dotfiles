...
const float DURATION = 0.10; // total animation time
const float TRAIL_SIZE = 0.74; // 0.0 = all corners move together. 1.0 = max smear (leading corners jump instantly)
const float THRESHOLD_MIN_DISTANCE = 1.5; // min distance to show trail (units of cursor height)
const float THRESHOLD_MAX_VERTICAL = 45.0; // max vertical jump to show trail (units of cursor height).
const float BLUR = 1.2; // blur size in pixels (for antialiasing)
const float TRAIL_THICKNESS = 1;  // 1.0 = full cursor height, 0.0 = zero height, >1.0 = funky aah
const float TRAIL_THICKNESS_X = 0.9;

const float FADE_ENABLED = 1; // 1.0 to enable fade gradient along the trail, 0.0 to disable
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float verticalDist = abs(centerCC.y - centerCP.y);
    float minDist = currentCursor.w * THRESHOLD_MIN_DISTANCE;
    float maxVertical = currentCursor.w * THRESHOLD_MAX_VERTICAL;

    vec4 newColor = vec4(fragColor);

    float baseProgress = iTime - iTimeCursorChange;

    if (lineLength > minDist && verticalDist < maxVertical && baseProgress < DURATION - 0.001) {
