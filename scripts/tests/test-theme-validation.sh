...
REQUIRED_TMUX_GENERATED=(
    # these are generated by theme-defaults.sh
    "TMUX_STATUS_BG"
    "TMUX_STATUS_FG"
...
REQUIRED_TMUX_DIRECT=(
    # these should be directly defined in theme files
    "TMUX_ACCENT_CYAN"
    "TMUX_ACCENT_PINK"
    "TMUX_ACCENT_PURPLE"
    "TMUX_FG_SECONDARY"
...
for theme_file in "$THEMES_DIR"/*.theme; do
    if [[ -f "$theme_file" ]]; then
        ...
        # check directly defined variables
        missing_direct=()
        for var in "${REQUIRED_TMUX_DIRECT[@]}"; do
            if ! grep -q "^$var=" "$theme_file"; then
                missing_direct+=("$var")
            fi
        done
        ...
        if [[ ${#missing_direct[@]} -eq 0 ]]; then
            pass "$theme_name: all directly-defined tmux variables present"
        else
            fail "$theme_name: missing directly-defined tmux variables: ${missing_direct[*]}"
        fi

        # check generated variables (after applying defaults)
        missing_generated=()
        for var in "${REQUIRED_TMUX_GENERATED[@]}"; do
            if ! (
                ...
                missing_generated+=("$var")
            fi
        done

        if [[ ${#missing_generated[@]} -eq 0 ]]; then
            pass "$theme_name: all generated tmux variables present after defaults"
        else
            fail "$theme_name: missing generated tmux variables: ${missing_generated[*]}"
        fi
    fi
done
...
for theme_file in "$THEMES_DIR"/*.theme; do
    if [[ -f "$theme_file" ]]; then
        ...
        # exclude non-colour metadata: THEME_NAME, NVIM_COLORSCHEME, THEME_ACTIVE_ACCENT
        invalid_colours=$(grep -E '^[A-Z_]+=' "$theme_file" | grep -vE '(="#?#[0-9a-fA-F]{6}"|="\$[A-Z_]+"|^#|^THEME_NAME|^THEME_ACTIVE_ACCENT|^NVIM_)' | head -5 || true)
...
for theme_file in "$THEMES_DIR"/*.theme; do
    if [[ -f "$theme_file" ]]; then
        ...
        # THEME_NAME is the filename with hyphens as spaces, title-cased
        defined_name=$(grep "^THEME_NAME=" "$theme_file" | sed 's/THEME_NAME="\([^"]*\)"/\1/')
        expected_name=$(printf '%s' "${theme_name//-/ }" |
            awk '{ for (i = 1; i <= NF; i++) $i = toupper(substr($i, 1, 1)) substr($i, 2); print }')

        if [[ -z "$defined_name" ]]; then
            fail "$theme_name: THEME_NAME not defined"
        elif [[ "$defined_name" == "$expected_name" ]]; then
            pass "$theme_name: THEME_NAME is '$defined_name'"
        else
            fail "$theme_name: THEME_NAME is '$defined_name', expected '$expected_name'"
        fi
    fi
done
