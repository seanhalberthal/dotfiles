...
- `zls` and `shfmt` are declared in Mason's install lists. Both were installed on this machine but named nowhere in the config, so a fresh install picked up neither. zls attached to zig buffers regardless, through mason-lspconfig's `automatic_enable` enabling every installed package whose name matches an lspconfig server, which meant the zig language server was live but not reproducible; shfmt was simply unused, with no filetype mapped to it. zls is pinned to `0.15.1`: it tracks zig's minor series and refuses to attach across one, and the registry carries only the latest, which is a series ahead of the zig installed here. The pin lives with the tool installer rather than beside the servers, whose `ensure_installed` takes no version and would pull the newest on a fresh machine. shfmt now formats `sh`, `bash` and `zsh`, all at `-i 4 -ci`; its default is tabs, and `-ci` keeps case bodies indented, which is how the scripts here are written, since without it every case arm is pulled back a level. Every tracked shell file is normalised to that in one pass, so later edits carry formatting-only churn into unrelated commits. The file list comes from `shfmt -f` intersected with `git ls-files`: shfmt's own directory walk ignores `.gitignore` and would rewrite the vendored TPM plugins under `tmux/plugins/`, which are untracked upstream code. The five associative-array keys in `scripts/dotfiles` that carry a space are quoted: shfmt reads an unquoted `[...]` subscript as arithmetic, so `[Zsh config]` was two identifiers with no operator between them and the whole file failed to parse. Quoting a subscript leaves the key untouched for an associative array, and the file formats like everything else now, which costs it the aligned one-line case arms it alone used. Case alternatives gain spaces around `|`, so the launcher test's exact-text assertion on `test|cd|ls` now accepts either spelling. `nvim/lua/custom/plugins/lsp.lua`
...
- `F6` pauses the debugger. Delve cancels an in-flight step when it takes a manual halt, so this is the only way to clear a step that a breakpoint interrupted without restarting the session. `nvim/lua/custom/plugins/debug.lua`, `nvim/cheatsheet.txt`

- A drift guard covering the uninstaller's symlink manifest. `uninstall.sh`'s `SYMLINKS` array carries a comment requiring it to match `create-symlinks.sh` exactly, and had fallen three entries behind it unnoticed. The guard resolves both sides with the same variables rather than comparing them as text, so `${XDG_CONFIG_HOME:-$HOME/.config}` and `$lazygit_dir` forms compare equal, and it checks every destination under both lazydocker layouts so the macOS path is covered on a Linux run. It fails in both directions: a `create_link` with no `SYMLINKS` entry, and an entry removed from `SYMLINKS`. `scripts/tests/test-uninstall.sh`

### Changed

- Python's language server is basedpyright rather than pyright. pyright ships no add-import quick fix at all: its whole command set is `createtypestub`/`organizeimports`/`restartserver`, and it offers import suggestions only as completion items, which additionally require the defining module to have been opened in the session before the symbol appears at all, so a two-file project gave nothing on `gra` and nothing in the completion menu until the other file had been visited. basedpyright reimplements the quick fix, and offers `from main import fib` with the defining module unopened. Its default `recommended` mode reports the inferred-Any rules (`reportAny`, `reportUnknownParameterType`) as warnings on ordinary untyped code, so `typeCheckingMode` is pinned to `standard`, the mode pyright ran in; the rest of the settings table comes from lspconfig and deep-merges with the pin. mason-lspconfig's `automatic_enable` enables every server installed under Mason rather than the configured list, so pyright has to be uninstalled (`:MasonUninstall pyright`) and not merely dropped from the list, or both attach and every undefined name is reported twice. `nvim/lua/custom/plugins/lsp.lua`, `nvim/lua/custom/features/scan-runner.lua`, `docs/TROUBLESHOOTING.md`, `zsh/dotfiles.zsh`

- ruff's `F821` is silenced. basedpyright reports the same undefined name as `reportUndefinedVariable`, so each one was drawn twice, once per server. The rule is ignored through `init_options.settings.lint.ignore` on the ruff server, which is configured directly rather than through the servers table since mason-lspconfig enables it off the installed `ruff` package rather than that list. ruff resolves client settings ahead of a project's `pyproject.toml` by default (`editorFirst`), so the ignore also holds in a project that selects the rule itself. Every other ruff rule is untouched. `nvim/lua/custom/plugins/lsp.lua`
...
- Swift files are linted. The executable guard in the nvim-lint config read `cmd` off the linter module directly, but a module is either the spec table or a factory returning one, and nvim-lint's swiftlint is a factory since it varies its args on whether a `.swiftlint.yml` is in play. `type(linter) == 'table'` was false for it, so the guard dropped swiftlint from the runnable list and `try_lint` was never called on a swift buffer: the CLI reports four violations on a file where the autocmd produced none. The factory is resolved before the check now. Python leaves `linters_by_ft` in the same change; the ruff language server is enabled by mason-lspconfig's `automatic_enable` (the Mason package name matches the lspconfig server name) and reports the same rule set the CLI does, so every python diagnostic was published twice, once by the server into `nvim.lsp.ruff.<id>.Ruff` and once by nvim-lint shelling out to the same binary into `ruff`. The server is the better of the two: it updates as you type rather than on write, and it carries the ruff code actions. `nvim/lua/kickstart/plugins/lint.lua`
...
- The cursor trail no longer antialiases on horizontal and vertical moves. The exemption was written as `float effectiveBlur = mix(0.0, BLUR, isDiagonal)` inside the guard block, which declares a second variable shadowing the one the `antialising` call reads and discards it at the closing brace, so the blur reaching the trail's shape mask stayed at `BLUR` in every direction and the documented fix had never run. Dropping the declaration assigns the outer variable instead. Axis-aligned moves take a hard edge again, which is what keeps the trail's alpha ramp off the cursor rectangle it collapses into at the end of the animation; diagonal moves are unchanged, and the `BLUR < 2.5` guard stays as the escape hatch for raising the constant. `ghostty/shaders/cursor-warp.glsl`
- `dotfiles theme <name>` runs the contrast check when it auto-generates a theme. `apply_theme` passed the flag as `${quiet:+--quiet}`, which expands when the variable is non-empty rather than when it is true, and the variable holds the literal strings `false` and `true`, so `--quiet` was sent on every call. `generate-theme` gates its post-generation contrast check on that flag, so a theme generated through the CLI was never validated and only a hand-run `scripts/generate-theme` reached the check. The flag is built by value now, matching what `generate-theme` already does internally. `scripts/theme-switch`
- `dotfiles set dev|projects` stores a path outside `$HOME` as written. The value was built as `"\$HOME${dir#"$HOME"}"`, an unconditional prefix strip, and `${var#pattern}` is a no-op when the pattern does not match, so `/opt/work` was written into `~/.zshrc` as `$HOME/opt/work` while the command reported the real path back. Paths under `$HOME` still store `$HOME`-relative so a synced `~/.zshrc` resolves on a machine with a different username; the test is an exact match or a `"$HOME"/*` glob, which also stops a sibling home sharing the prefix (`/Users/name2/x` under `HOME=/Users/name`) being rewritten as `$HOME2/x`. `scripts/dotfiles`
- A successful install clears its rollback state. `cleanup_rollback_state` was called only inside the `--check-only` branch, so `.install-state/` survived every real install, and `scripts/install/rollback.sh` branches on that state existing: it removes every recorded symlink first, then finds `backup-location.txt` empty and reports "No backup found to restore" having already unlinked them. The `~/.dotfiles-backup` restore that would have worked is offered only when no state is present, so stale state turned a recoverable rollback into a destructive one. The call now sits where its comment always claimed it did, after the last mutating step and before the reporting that follows. `install.sh`
- `--check-only` no longer installs anything. It ran Homebrew setup and `brew bundle install --upgrade` before its early exit, so `make check` upgraded every package on the machine while `install.sh --help`, `health-check.sh`, the Makefile target and the installation guide all described it as a dry run. The two brew steps are gated on the flag directly rather than by reusing `--skip-brew`, so the skip lines name the flag that was actually passed, and a prerequisite failure under `--check-only` points at `./install.sh --core` rather than suggesting a `--skip-brew` that was never given. The prerequisite and health checks still run, and still exit non-zero when tools are missing. `install.sh`
- Uninstall removes the ImageMagick font map, the lazydocker log formatter and the statusline theme resolver. The `SYMLINKS` array carries a comment requiring it to match `create-symlinks.sh` exactly and was missing those three, so each survived uninstall as a link into a `~/dotfiles` it had just finished detaching from. lazydocker's destination differs by platform and the ImageMagick map is installed on macOS only, so both are appended under a `Darwin` test beside the existing legacy Ghostty entry; the preview and removal loops both skip a path that is not a symlink, so listing the Linux lazydocker path unconditionally costs nothing. `scripts/install/uninstall.sh`
- Stepping through a Go subtest no longer wedges the session. Stepping off the end of a `t.Run` closure walks into `testing.tRunner` and then into `runtime.goexit`, and a step taken from there ends the goroutine mid-step: delve conditions its stepping breakpoints on that goroutine id, so they can never fire, and it keeps them installed because the next breakpoint hit lands on a different goroutine. Every later step then fails with "next while nexting", surfaced as `Unable to step while the previous step is interrupted by a breakpoint`, and continue does not clear it because the step it is told to resume belongs to a goroutine that no longer exists. `F1`/`F2`/`F3` now check the top frame first and refuse to step when it is `runtime.goexit`, which is the frame delve reports for that state. Other routes into the same wedge remain (a step over a `t.Fatalf`, or a stepping goroutine that finishes while you are stopped elsewhere); `F6` is the escape hatch for those. `nvim/lua/custom/plugins/debug.lua`
- `r` in copy-mode works. It sent `refresh-on`, which is not a copy-mode command: `window_copy_cmd_table` has `refresh-from-pane` and nothing by that name, and an unknown command falls through to `WINDOW_COPY_CMD_NOTHING` without an error, so the key cleared the selection and did nothing else. It also replaced tmux's own default binding for that key, which is the same `refresh-from-pane` it should have been calling. The explicit `clear-selection` stays: `refresh-from-pane` carries `CLEAR_ALWAYS`, but that governs search marks rather than the selection. `tmux/tmux.conf.template`
- nvim no longer prints `Theme: <name>` on every start, which was reaching stderr on any headless or scripted invocation. The notification was guarded on `force or current_theme ~= nil`, evaluated after `current_theme` had already been assigned, so it could never be false. The guard could not have worked as written either: `M.reload` returns early unless the reload is forced or the theme actually differs, so "forced or changed" is guaranteed by the time it runs. The real gap was that `current_theme` starts nil, making the first apply of a session look like a change; the previous value is now captured before the assignment. `nvim/lua/custom/core/theme.lua`
- The lazydocker config no longer claims dotfiles will overwrite it. It is installed with `copy_config`, which writes only when the destination is absent, so the file has always been user-owned after the first install and repo changes have never propagated to it. Existing installs keep the old header, since that is precisely what the corrected header now says. `lazydocker/config.yml`
- The `md` suffix alias runs glow. A suffix alias prepends its value to the filename, and the value was written `-t glow`, so typing a bare `notes.md` ran `-t glow notes.md` and died with `command not found: -t`. `-t` is glow's `--tui` flag, so the value is `glow -t`. The trailing description still does not reach `dotfiles aliases`: the parser matches `^[[:space:]]*alias [a-zA-Z_]`, which `alias -s md=` never satisfies, so listing it would need a `# @cheat:` directive instead. `zsh/dotfiles.zsh`
- `update_zshrc_export` keeps backslashes in a value it writes for the first time. The function computes an escaped copy of the value for its `s///` replace branch, but the append branch that runs when the export does not yet exist interpolated the raw value into a `sed` `a\` text block, where sed consumes a backslash, so `a\b` landed as `ab`. Only that one path was affected: `&` is not special in `a\` text, and the marker-less append hands the value to `printf` as a `%s` argument rather than as format. The append now doubles backslashes, and a regression test covers it. `scripts/_lib/common.sh`, `scripts/tests/test-linux-compat.sh`
- Cmd+Backspace deletes to the start of the line again instead of killing the whole line. Ghostty emitted `\x15` for `super+backspace`, which is plain `^U`, and zsh's emacs default binds `^U` to `kill-whole-line`; the `\e[127;5u` bindkey that maps to `backward-kill-line` was therefore unreachable, as were the nvim `<C-BS>` mappings beside it. Ghostty emits the csi-u form again, which tmux forwards unchanged under `extended-keys always` with `extended-keys-format csi-u` (verified: the sequence reaches the pane byte for byte). Sending a distinct sequence rather than rebinding `^U` keeps `^U` itself as `kill-whole-line` at the prompt, and keeps Cmd+Backspace out of the way of applications that scroll on `^U`. The csi-u form was originally introduced deliberately and then reverted as an unintended change, which left the zsh bindkey and both nvim mappings in place with nothing emitting the sequence they matched. `ghostty/config.template`
- The local-layer manifest drift guard checks destinations rather than source paths. `CLAUDE.md` documents it as failing when an `install_local`/`copy_config` destination is missing from `_local_pairs`, but it extracted the installer's *source* argument and substring-matched it against the whole manifest blob; since manifest lines are `repo_path|destination`, a match against the repo-path half satisfied it and the destination was never read. Corrupting only a destination left the guard green. It now resolves the installer's destination expression with the variables `create-symlinks.sh` has in scope and requires a whole-line match against the manifest's own destinations, which `_local_pairs` already emits resolved. The reverse direction stays unchecked, so a manifest entry with no installer counterpart is still fine. `scripts/tests/test-dotfiles-local.sh`
- Three tests that could not fail now can. `test-create-symlinks-functional.sh` claimed to exercise the local-override logic but never called `install_local`: it wrote a string and asserted the same string, so it passed with the function deleted. It now sources `symlink.sh` and calls the real function on both branches, the existing file it must not overwrite and the missing one it must seed. `test-uninstall.sh`'s non-symlink check created a file and asserted the file existed, with the removal loop nowhere in sight; the real file now goes through the loop, so the `[[ -L ]]` guard is what preserves it. `test-theme-validation.sh` said it checked `THEME_NAME` against the filename and asserted only that it was non-empty, which a theme naming itself after a different theme would pass; the filename-to-title-case rule holds for all fourteen themes, so it is checked directly. `scripts/tests/test-create-symlinks-functional.sh`, `scripts/tests/test-uninstall.sh`, `scripts/tests/test-theme-validation.sh`
- `restore_from_backup` is tested by calling it. The suite's "path traversal protection" section never invoked the function: it read `rollback.sh` as text and asserted the literals `../*` and `/../` appeared in it, which a comment inside the test admitted was structural. The function's actual contract, that a backup tree is copied into `$HOME` at the same relative paths, was untested. It now restores a real backup in the sandboxed `$HOME` and checks a top-level file, a nested one, and that a symlink at the destination is replaced rather than written through. The traversal assertion stays as a source-level regression marker, relabelled to say so: `relative_path` is `find` output with the `backup_dir` prefix stripped, which cannot produce a `../` or `/./` component, so no input reaching the function can exercise that guard. `scripts/tests/test-rollback-lib.sh`
- Three weaker test assertions tightened. The theme-installation suite checked that theme-switch output is suppressed by searching the whole of `create-symlinks.sh` for `--quiet` or a redirect, so any unrelated redirect satisfied it; it now anchors to the `theme-switch` invocation lines themselves and requires every one of them to be quiet. `test-theme-switch.sh` ran the same `grep -q reload-ghostty.sh` twice under two different messages, so the duplicate is gone. `test-fzf-theming.sh` named a `pick-theme.sh` that does not exist, in its comment and both result messages, while asserting against the real `themes/pick.sh`. `scripts/tests/test-theme-installation.sh`, `scripts/tests/test-theme-switch.sh`, `scripts/tests/test-fzf-theming.sh`

### Removed

- The roslyn.nvim#371 pull-diagnostics shim. It wrapped `vim.lsp.diagnostic.on_diagnostic` globally, for every client and every diagnostic response, to guard a crash caused by `projectInitializationComplete` calling `_refresh()` with no bufnr. The issue closed upstream and the pinned plugin now iterates `attached_buffers` and passes an explicit bufnr, so the nil the guard existed for cannot occur. Removing it also orphaned the module's `require` in `lsp.lua`, since the other export is reached through an inline require elsewhere. `nvim/lua/custom/features/roslyn-diagnostics.lua`, `nvim/lua/custom/plugins/lsp.lua`, `nvim/lua/custom/features/lsp-patches.lua`
- The `terminal-overrides` entry remapping Home/End to CSI sequences. tmux builds its input key table from two independent sources, and only the terminfo-driven one is affected by `terminal-overrides`; the unconditional table already maps `\E[H` and `\E[F`, which is exactly what Ghostty sends for cmd+left/right. The override targeted `xterm-256color`, which never matched the `xterm-ghostty` the client actually reports, so it was inert under Ghostty and redundant everywhere else. `tmux/tmux.conf.template`
- The `setreg('+')` call in the yank autocmd, which re-wrote a value `clipboard=unnamedplus` had already placed in the register. The comments around it described a decoupled clipboard where only yanks synced; that stopped being true when `unnamedplus` was reinstated for cross-instance yank sharing, and deletes have reached the system clipboard since. Behaviour is unchanged; `<leader>v` (`"0p`) remains the way back to the last yank after a delete. `nvim/lua/custom/core/autocmds.lua`, `nvim/lua/custom/core/keymaps.lua`
...
- `<leader>bw` adds a dap-view watch expression without leaving the source buffer: the expression under the cursor in normal mode, the selection in visual mode, and the panel switches to its watches view either way. A watch previously meant moving into the panel, reaching the watches section and pressing `a` there. `o` is bound alongside `<CR>` in every section as well, so one key expands a variable, jumps to a frame or a breakpoint, and toggles an exception filter or a session, rather than each section answering to a different one; threads' own `o` (invert_filter) moves to `O`. The cheatsheet's debug rows are rewritten around the panel as it now behaves, replacing the two scopes-only entries that predate dap-view. `nvim/lua/custom/plugins/debug.lua`, `nvim/cheatsheet.txt`
...
- `:grep`, along with `grepadd`, `lgrep` and `lgrepadd`, runs silently and opens the list it fills. `:grep` shells `grepprg` out through `:!`, so it echoed the command line with its tmpfile path, then every raw ripgrep match, then the jump message, and finished on a hit-enter prompt; under `cmdheight=0` that forces the cmdline area open for the length of the output and collapses it again. A cmdline abbreviation prepends `silent`, and a `QuickFixCmdPost` hook runs `cwindow` (`lwindow` for the location-list variants), which opens the window only when there are entries and closes a stale one when there aren't. The abbreviation carries no bang of its own: an abbreviation fires on the non-keyword character that follows it, so `:grep!` expands at the `!`, and a baked-in bang would yield `:grep!!`, which searches for `!` and takes the real pattern as a path. `:grep` therefore still jumps to the first match and `:grep!` still doesn't. The expansion is guarded on the cmdline being nothing but the command, so `:vimgrep`, `:Ggrep` and an explicit `:silent grep` are left alone. Silencing swallows the zero-match case whole, since a `grepprg`-based `:grep` reports no `E480`, so an empty result notifies instead. `nvim/lua/custom/features/lists.lua`, `nvim/cheatsheet.txt`
- `listchars` renders tabs blank instead of `» `. indent-blankline already paints leading indentation with its own `tab_char`, so the `listchars` glyph only ever surfaced where ibl doesn't reach: mid-line tabs, and commented-out code, where prepending `// ` takes the indentation out of leading position and unmasks the arrows for a whole commented block. The value is two spaces rather than an omitted key; with `list` set, dropping `tab` falls back to `^I`. `nvim/lua/custom/core/options.lua`
- Go's `tdt` table-driven test snippet uses the `tests`/`tt`/`name` naming from the standard library's own tests rather than friendly-snippets' `testCases`/`tC`/`desc`. It is the first entry in `nvim/snippets/`, which blink's default preset already scans ahead of friendly-snippets but never deduplicated against it: `sources.deduplicate` is unimplemented upstream and `filter_snippets` only drops whole files, so a local override would have sat in the menu next to the copy it replaces. A `transform_items` on the snippets provider keeps the first item for each label, which is the local one wherever a prefix collides, so a single snippet can be replaced without taking over the filetype. Its only other effect is on markdown, where it collapses the nine entries `global.json` contributes twice by being registered for both `all` and `markdown`. `nvim/snippets/go.json`, `nvim/lua/custom/plugins/completion.lua`
- The buffer completion source waits for a four-character keyword. `min_keyword_length` was 0 for every source, so a three-character query was handed to the fuzzy matcher against every word in the buffer, where the typo budget (`floor(#keyword / 4)`) is at its most permissive relative to the query: typing `Env` in a Go struct offered `received`, scraped out of an unrelated log string, as the only candidate. Since `preselect` is on, that lone match was selected, and both `<CR>` and `<Tab>` accept the selection, so the noise landed in the buffer rather than sitting there ignorable. Short prefixes are the case the lsp already serves best, so nothing useful is given up. `nvim/lua/custom/plugins/completion.lua`
- Go import paths render in one colour. gopls tokenises the path as a `namespace`, which nvim links to `@module`, but the token covers the path text without the quotes around it, so `"fmt"` drew its quotes in the string colour and its contents in the module colour. An after-query captures the whole literal as `@module`, which also lines the import block up with the package qualifier at the use site: nvim-treesitter's Go query leaves `fmt` in `fmt.Printf` as `@variable`, so gopls's token is the only thing colouring it, and turning the type off to even the import block up would have flattened every qualifier instead. `nvim/after/queries/go/highlights.scm`
- A failed test no longer reads as a broken file. neotest's diagnostic consumer publishes every failure as a real ERROR diagnostic in its own `neotest` namespace, so a red suite drew an error squiggle under the assertion, counted into the statusline's `E` total next to anything gopls had reported, and landed in the `<leader>xx` diagnostics list alongside genuine build problems. A namespace-scoped `vim.diagnostic.config` turns `underline` and `signs` off for that namespace alone, leaving the message itself on the failing line through `virtual_lines`; neotest's own ✗ status sign already owns the gutter cell, at priority 1000 against the diagnostic sign's 10. The statusline subtracts the namespace from each severity count and renders the total separately as `✗N` in `NeotestFailed`'s colour, and `diags_to_items` drops it, which covers `<leader>xx` and `<leader>xX` together. Setting neotest's own `diagnostic.severity` would not have done it: neotest-golang classifies each error line itself and passes an explicit severity, so the consumer's config value is only ever a fallback for adapters that don't. `nvim/lua/custom/plugins/test.lua`, `nvim/lua/custom/features/statusline.lua`, `nvim/lua/custom/features/lists.lua`
- neotest's quickfix consumer is off. It is on by default and pushes a fresh untitled list on every failing run, which drops the forward half of the quickfix stack and pauses the live `<leader>xx` diagnostics sync, since that only rebuilds while the current list is the one it titled `Diagnostics: all`. Nothing is lost by it: the summary window, the status signs and `]t`/`[t` already carry the failures and the movement between them. `nvim/lua/custom/plugins/test.lua`
- The `dev` launcher opens two windows rather than three, and lands on the editor. Its third window ran `claude` at session creation, so every session started an agent process whether it was going to be used or not, and picked which agent months in advance; `prefix + n` creates one on demand and asks which, so the launcher no longer decides that up front. What is left is the shell and nvim, and the initial selection moves from window 1 to the nvim window, addressed by name so it doesn't depend on the index it happens to land on. The `config` and `dotfiles` launchers describe their window as oil rather than Neo-tree, which is what `nvim .` has opened since oil took `default_file_explorer`; neo-tree is still there, on `|`. `launchers/dev`, `launchers/config`, `launchers/dotfiles`
...
- The dashboard's statusline hide follows window focus rather than firing once on open. `laststatus` is global, so a one-shot toggle leaks in both directions: snacks' own startup hide gives up the first time another window is entered and never comes back, which is why closing a `:Differ` tabpage landed on a dashboard with a statusline drawn over it, while hiding on `SnacksDashboardOpened` instead followed the dashboard into every other window. A `BufEnter`/`WinEnter` watcher now syncs the option to whether the focused window holds the dashboard buffer, and it is created and torn down with the dashboard itself, so ordinary editing carries no extra per-event hook. Floating windows are skipped, so a picker opened over the dashboard doesn't flip the statusline back on behind it. `nvim/lua/custom/plugins/dashboard.lua`
- Spell autocorrect restores `wrapscan` and the saved view even when the range walk throws. `wrapscan` is global and is cleared for the duration so `]s` can't wrap past the range, so a throw partway through (an out-of-range cursor set, say) left the whole session without search wrapping and skipped the `winrestview` on the way out. The walk moved into a local function called through `pcall`, with both restores on the common path and the error rethrown afterwards. `nvim/lua/custom/core/spellcheck.lua`
- Dismissing the completion menu no longer commits the item that was previewed into the buffer. `completion.list.selection.auto_insert` writes the selected item into the buffer as the list is cycled, and blink's `hide` command only closes the menu: it leaves that preview behind, so `<C-e>` on an unwanted suggestion kept the suggestion and discarded what had been typed. `cancel` undoes the preview before hiding, which is what blink's own `default` preset binds `<C-e>` to and what the `auto_insert` documentation asks for; `hide` predated that change upstream. `<M-CR>`, which inserts a literal newline without accepting, took the same route and is switched with it. `<Esc>` still leaves the preview in place, since it leaves insert mode without going through blink. `nvim/lua/custom/plugins/completion.lua`, `nvim/cheatsheet.txt`
- Escape sequences and format verbs are highlighted inside Go strings again. gopls sends one `string` semantic token per literal, escapes included, at semantic-token priority 125, which covers treesitter's `@string.escape` at 100, so `"...\n"` rendered as one flat colour. It surfaced when mason moved gopls from v0.22.0 to v0.23.0: v0.22.0 sends no semantic tokens at all and leaves the buffer to treesitter, while v0.23.0 honours the `semanticTokens = true` that nvim-lspconfig sets client-side because gopls stopped advertising the capability by default in v0.22.0. `semanticTokenTypes = { string = false }` drops that one token type and leaves every other gopls token in place; all it carried that treesitter does not is a `format` modifier on printf verbs, which no theme here styles. With the literal back under treesitter, the `regex` parser makes the injection go's `injections.scm` already declares live, so a `regexp.MustCompile` argument highlights its own anchors, classes and quantifiers rather than reading as one flat literal. The `printf` parser that the same file injects into format strings is deliberately not installed: it is a C grammar, and go's `%t` and `%w` are C length modifiers rather than conversions, so a format node runs on from either of them to the next conversion character and swallows the text in between. Verbs are marked by `custom.features.go-format-verbs` instead, which walks the go tree for the string content of the printf-family calls vet itself checks and scans it for go's own verb set, flags, argument indexes, widths and precisions included. No query can do that job: treesitter captures whole nodes, and a format string is a single token to the go grammar. `nvim/lua/custom/plugins/lsp.lua`, `nvim/lua/custom/plugins/treesitter.lua`, `nvim/lua/custom/features/go-format-verbs.lua`
- The instance pickers create a window again in sessions that hold a window named after the session. `new-window -t <name>` takes a target-window rather than a target-session, and tmux resolves a window carrying that name ahead of the session itself, pinning the new window to an index that is by definition occupied and failing with "index in use"; `automatic-rename-format` names a plain zsh pane after the basename of its cwd, so every session named after its project directory grows exactly that collision. `new.sh` runs under fzf's `execute-silent` with `set -e`, so the error reached neither the popup nor the status line, and `prefix + n`, along with the claude, codex, opencode and copilot pickers behind the same script, did nothing at all. The target is now the session id with a trailing `:`, which survives both the collision and a later rename; the new window is tracked by `#{window_id}` instead of `session:index`, so `renumber-windows` cannot shift it out from under the `send-keys` that follows; and a failed `new-window` reports through `show_error` rather than exiting silently. Window duplication and generated launchers carried the same bare-session target and are fixed with it. Launchers already generated under `~/.config/dotfiles/launchers/` are user-owned, so they keep the old form until the `-t "$SESSION"` in each `tmux new-window` line is given the trailing colon by hand. `tmux/scripts/instances/new.sh`, `tmux/scripts/windows/duplicate.sh`, `tmux/scripts/launchers/new.sh`, `launchers/launcher.template`, `launchers/dev`, `tmux/scripts/tests/test-instance-new.sh`
- Go files parse again under go 1.26's `new(expr)`. The go grammar hardcodes the first argument to `new` and `make` as a type, so `new(0)` is a syntax error to the parser, and past the first few the file collapses into `ERROR` nodes, taking highlighting, folds and neotest's test discovery with it. The `go` parser is pinned to the head of the open upstream pr for it (tree-sitter-go#193), one commit on top of the revision nvim-treesitter already pins, fetched from `tree-sitter/tree-sitter-go` rather than the fork it lives on, since upstream serves a pr head through its own refs and the pin survives the fork being deleted or renamed. `sync_pins` rewrites `install_info` and hands the table back through `package.preload`, which is checked ahead of the path searchers: nvim-treesitter's `install()` drops the parser table from `package.loaded` and re-requires it before resolving a source, so a plain mutation would not survive. The compiled `.so` and any installed queries are dropped whenever the recorded revision changes, since the install probe only reinstalls a parser it cannot find, and a marker with no matching pin purges the same way, so removing an entry restores the upstream parser rather than leaving the pinned build in place. The pin costs `new[i]` on a variable shadowing the builtin, which the grammar mis-parses in the other direction (tree-sitter-go#189). `nvim/lua/custom/features/treesitter-parsers.lua`, `nvim/lua/custom/plugins/treesitter.lua`
- Table-driven test cases reach the neotest summary in full, and follow the table as it is edited. Three separate things stood in the way. nvim parses with a 256 in-progress tree-sitter match cap, and past it the earliest-starting matches are dropped, so a long table was silently truncated to its tail: 43 cases for a keyed `tt := []struct{}` table, 84 for map and unkeyed shapes. `parse_positions` takes a `match_limit`, but no adapter passes one, so it is wrapped to raise it. neotest-golang then keys its per-file discovery cache on whole-second mtime and expires nothing afterwards, so a second write inside the same second was served the stale tree for good; the key becomes the full `sec:nsec:size` stat signature, captured before the parse rather than after, so a write landing mid-parse costs a re-parse instead of stranding stale positions. And the autosave handler's writes emitted no write events at all: autocmds don't nest, so its `silent! write` fired no `BufWritePost`, which is where neotest re-discovers a file's positions, and a deleted case stayed in the summary until the buffer was reopened. The event is re-emitted by hand behind a reentrancy guard rather than marking the autocmd `nested`, which would also re-run `BufWritePre`, and with it format-on-save, on every keystroke-level edit. neotest-golang's duplicate-subtest-name warning goes off with them: it `vim.notify`s on every discovery, which was once per file open and is now once per save, so a table with two cases briefly sharing a name notifies as it is typed. `nvim/lua/custom/plugins/test.lua`, `nvim/lua/custom/core/autocmds.lua`
...
- The noice signature help popup is capped at 80 columns by 5 rows and no longer offsets to the right of the cursor. noice has no view of its own for signature help, so it borrows the hover view: 120 by 20, anchored to the cursor, and offset two columns right by the `lsp_doc_border` preset. Since the formatter appends the whole documentation body after the call signature, a function like `nvim_create_autocmd` fills every one of those rows, and the result lands over the code the moment the opening paren is typed. The cap keeps the signature and the active parameter, which is the part worth seeing mid-call, and the rest stays reachable by scrolling. The overrides are per-kind, so `K` hover keeps the full 120 by 20. `nvim/lua/custom/plugins/ui.lua`
...
- copilot's language server no longer outlives nvim. `exit_timeout` defaults to `false`, and on that default nvim's `VimLeavePre` handler neither force-stops the server nor waits for it: it fires an async shutdown request, calls `vim.wait(0)`, then exits, so a server too slow to answer inside that window reparents to pid 1 still holding its resident memory. Under memory pressure that is every session, and the orphans compound the pressure that caused them. A number instead defers a force-stop and makes the handler wait for it, returning as soon as the server closes; 1000ms is long enough for a healthy exit and short enough that nvim's "waiting for LSP exit" notice, which appears after 100ms, stays rare. `nvim/lua/custom/plugins/copilot.lua`
...
- aerc is no longer managed by the dotfiles. The `aerc/` configs, the installer's core-tier symlink step, the `aerc` wrapper alias, the `accounts.conf` entry in the local-layer manifest, and the chmod-600 special case in `dotfiles import` all go, along with the `aerc` and `w3m` formulae; `w3m` was present only for aerc's html filter and nothing else depends on it. A migration removes the two symlinks that now dangle, but only where they are genuinely broken links into an aerc path, so a hand-rolled config is left alone. `accounts.conf` is kept and reported rather than deleted, since it is user-owned and holds account details, and the formulae are left installed for the same reason. `Brewfile`, `zsh/dotfiles.zsh`, `scripts/install/create-symlinks.sh`, `scripts/_lib/local-layer.sh`, `scripts/dotfiles`, `scripts/tests/test-dotfiles-local.sh`, `scripts/migrations/0.2.138-remove-aerc.sh`
...
- Session-collision detection works again for parameterised launchers. The `dev` launcher stopped suffixing `-dev` on derived session names in 0.2.92, but the picker kept deriving `<basename>-<launcher>` when checking for an existing session, so the check never matched and launching into a directory that already had a session skipped the collision prompt. Parameterised launchers all derive the session from the directory basename (the scaffold template produces fixed-session launchers), so the picker now uses the bare sanitised basename. `tmux/scripts/launchers/run.sh`, `launchers/dev`
- Diagnostics on generated files no longer fill the nvim lists. Generated code (sqlc, protoc-gen-go, mockgen, stringer) carries the Go canonical `// Code generated ... DO NOT EDIT.` sentinel on an early line; it is regenerated wholesale and never hand-edited, so findings on it are noise. The check sits next to the existing library/vendor filter and is shared by the live `<leader>xx` list and the git-scoped scans, cached per buffer against `changedtick`. The live list also collapses rows on file, line, column and text: `vim.diagnostic.get(nil)` spans every buffer, so the same file open under two bufnrs (a diff/review split, a second-window reopen) yielded byte-identical duplicates. `nvim/lua/custom/features/scan-runner.lua`, `nvim/lua/custom/features/lists.lua`, `nvim/lua/custom/features/diag-scan.lua`
