...
- `zls` and `shfmt` are declared in Mason's install lists. Both were installed on this machine but named nowhere in the config, so a fresh install picked up neither. zls attached to zig buffers regardless, through mason-lspconfig's `automatic_enable` enabling every installed package whose name matches an lspconfig server, which meant the zig language server was live but not reproducible; shfmt was simply unused, with no filetype mapped to it. zls is pinned to `0.15.1`: it tracks zig's minor series and refuses to attach across one, and the registry carries only the latest, which is a series ahead of the zig installed here. The pin lives with the tool installer rather than beside the servers, whose `ensure_installed` takes no version and would pull the newest on a fresh machine. shfmt now formats `sh`, `bash` and `zsh`. Its default indent is tabs and the two dialects here disagree, scripts at four spaces and `.zsh` at two, so `sh`/`bash` take `-i 4 -ci` and zsh runs through a second formatter entry at `-i 2 -ci`. `-ci` keeps case bodies indented, which is how the scripts here are written; without it every case arm is pulled back a level. `nvim/lua/custom/plugins/lsp.lua`
